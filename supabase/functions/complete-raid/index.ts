...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const packWeight=ids.filter(id=>id!=='portable_generator').reduce((n,id)=>n+catalog[id].weight,0);if(packWeight>52.05)throw new Error('Field pack weight exceeded.')
