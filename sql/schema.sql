...
-- Build 2.6.12: persistent per-instance ammunition / magazine contents.
alter table public.inventory add column if not exists item_data jsonb not null default '{}'::jsonb;
...
begin
  ...
  from jsonb_to_recordset(p_items) as x(item_id text,item_name text,value integer,weight numeric,rarity text,item_data jsonb);
